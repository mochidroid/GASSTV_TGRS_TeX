...
\begin{equation}
	...
	\AlgoLfloor{
		\begin{array}{l}
			\VarPrimal{1}^{(\IdxAlg+1)} 
			\leftarrow \prox_{\ScalarStepsize{1, 1}, \FuncPrimal{1}}
			\left( \VarPrimal{1}^{(\IdxAlg)} - \ScalarStepsize{1,1} \bigl( \sum_{\IndexDual=1}^{\NumVarDual} \LinOpPPDS{\IndexDual,1}^{\top} \VarDual{\IndexDual}^{(\IdxAlg)} \bigr) \right), \\
			\vdots \\
			\VarPrimal{\NumVarPrimal}^{(\IdxAlg+1)} 
			\leftarrow \prox_{\ScalarStepsize{1, \NumVarPrimal}, \FuncPrimal{\NumVarPrimal}}
			\left( \VarPrimal{\NumVarPrimal}^{(\IdxAlg)} - \ScalarStepsize{1, \NumVarPrimal} \bigl( \sum_{\IndexDual=1}^{\NumVarDual} \LinOpPPDS{\IndexDual,\NumVarPrimal}^{\top} \VarDual{\IndexDual}^{(\IdxAlg)} \bigr) \right), \\
			\VarPrimal{\IndexPrimal}^{'} = 2\VarPrimal{\IndexPrimal}^{(\IdxAlg+1)} - \VarPrimal{\IndexPrimal}^{(\IdxAlg)} \: (\forall \IndexPrimal = 1, \ldots, \NumVarPrimal), \\
			\VarDual{1}^{(\IdxAlg+1)}
			\leftarrow \prox_{\ScalarStepsize{2, 1}, \FuncDual{1}^{*}}
			\left( \VarDual{1}^{(\IdxAlg)} - \ScalarStepsize{2, 1}
			\bigl(\sum_{\IndexPrimal=1}^{\NumVarPrimal} \LinOpPPDS{1,\IndexPrimal} \VarPrimal{\IndexPrimal}^{'} \bigr) \right), \\
			\vdots \\
			\VarDual{\NumVarDual}^{(\IdxAlg+1)}
			\leftarrow \prox_{\ScalarStepsize{2, \NumVarDual}, \FuncDual{\NumVarDual}^{*}}
			\left( \VarDual{\NumVarDual}^{(\IdxAlg)} - \ScalarStepsize{2, \NumVarDual}
...
\begin{thm}
	...
	\end{equation}
	Then, the sequence $\lbrace \VarPrimal{1}^{(\IdxAlg)}, \ldots, \VarPrimal{\NumVarPrimal}^{(\IdxAlg)}, \VarDual{1}^{(\IdxAlg)}, \ldots, \VarDual{\NumVarDual}^{(\IdxAlg)} \rbrace$ generated by the procedure in \eqref{algo:P-PDS} converges to an optimal solution of Prob.~\eqref{prob:convex_optim_prob}.
\end{thm}
