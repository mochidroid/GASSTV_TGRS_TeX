...
\def\FuncPrimal{f}
\def\FuncDual{g}
\def\FuncSmooth{\varphi}
\def\VarPrimal{\mathbf{x}}
\def\VarDual{\mathbf{y}}

\def\LinOpPDS{\mathbf{A}}

\def\DimVarPrimal{n}
\def\DimVarDual{m}

\def\LipsitzConst{\beta}

\def\MaxSingularValue#1{\sigma_{\text{max}} (#1)}
...
\def\SpatialGraphWeightMat{\mathbf{W}_{\SpatialGraph}}
\def\SpatialGraphWeightMatTwoD{\mathbf{W}_{\text{sp}}^{'}}
\def\IndexEdgeOne{p}
\def\IndexEdgeTwo{q}
...
\def\SpatialGraphIncidenceMat{\mathbf{D}_{\SpatialGraph}}
\def\SpatialGraphIncidenceMatTwoD{\mathbf{D}}
...
\def\Centroid#1{\mu_{#1}}
\def\LabelMap{\mathbf{L}}
\def\SpectrumRep#1{\mathbf{r}_{#1}}

\def\VecSpecDiff#1{\mathbf{d}_{#1}}
\def\IdxSpecNodeOne{p}
\def\IdxSpecNodeTwo{q}

\def\SpectralGraphSegment#1{\mathcal{G}_{\lambda}^{(#1)}}

\def\NumNearestNeighbor{M}
\def\SpecGraphWeightSeg#1#2{w_{#2}^{(#1)}}
\def\SpecGraphWeightSegMat{\mathbf{W}_{\lambda}}
\def\SpecGraphDegreeSeg#1{\mathbf{D}^{(#1)}}

\def\IdcNN#1{\mathcal{N}_{#1}}
...
\def\ParamSpecGraph{\sigma_{\lambda}}

\def\DiffOpGraphBand{\mathbf{D}_{\SpectralGraph}}

\def\SpecGraphLaplacianSeg#1{\mathbf{L}^{(#1)}}
\def\SpecGraphLaplacian{\mathbf{L}_{\SpectralGraph}}
...
\def\BalanceParam{\omega}
\def\BalanceParamOne{\omega_{1}}
\def\BalanceParamTwo{\omega_{2}}

\def\RadiusGraphSp{\rho_{\text{sp}}}
\def\ParamRadiusGraphSp{\omega_{\text{sp}}}



% 3.3 HSI Denoising Problem
\def\BallSpatialGraph{B_{1, \RadiusGraphSp}}
