...
%\def\SmallBlock#1{\mathbf{L}_{\HSIClean}^{(#1)}}
\def\SmallBlock#1{\mathbf{T}_{\HSIClean}^{(#1)}}
%\def\NumBlock{B}
\def\NumBlock{L}
\def\IndexBlockPre{b}
\def\IndexBlock{l}
\def\NumBlockPixel{N^{'}}
\def\NumBlockVert{N_{1}^{'}}
\def\NumBlockHori{N_{2}^{'}}
...
% 3.1 S3TTV
\def\IndexBand{\IndexPrimal}

\def\DiffOpVert{\mathbf{D}_{v}}
\def\DiffOpHori{\mathbf{D}_{h}}
\def\DiffOpBand{\mathbf{D}_{s}}
\def\DiffOpSp{\mathbf{D}}

\def\DiffOpVertT{\mathbf{D}_{v}^{\top}}
\def\DiffOpHoriT{\mathbf{D}_{h}^{\top}}
\def\DiffOpBandT{\mathbf{D}_{s}^{\top}}
\def\DiffOpSpT{\mathbf{D}^{\top}}

\def\ExpantionOp#1{\mathbf{P}_{#1}}




\def\LeftSingularMatrix{\mathbf{U}}
\def\SingularMatrix#1{\mathbf{\Sigma}_{#1}}
\def\RightSingularMatrixT{\mathbf{V}^{\top}}
\def\SingularValue#1{\rho_{#1}}
...
\def\IndexSTV{\text{(g)}}
\def\IndexSSST{\text{(h)}}
\def\IndexLRTDTV{\text{(i)}}
\def\IndexFGSLR{\text{(j)}}
\def\IndexTPTV{\text{(k)}}
\def\IndexQRNND{\text{(l)}}
\def\IndexFastHyMix{\text{(m)}}
\def\IndexSSSTTV{\text{(n)}}


% Appendix
\def\MatHSIClean{\mathbf{U}^{(\IndexBlock)}}
